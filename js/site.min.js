...
$("section code").each(function(b){$(this).html($(this).html().replace(/^\s*\n/,""))});$("section img").each(function(b){$(this).parent().is("a")||$(this).wrap('<a href="'+$(this).attr("src")+'" title="Click to see original" target="_blank"></a>')})});var metas=document.getElementsByTagName("meta"),i;
if(navigator.userAgent.match(/iPhone/i)){for(i=0;i<metas.length;i++)"viewport"==metas[i].name&&(metas[i].content="width=device-width, minimum-scale=1.0, maximum-scale=1.0");document.addEventListener("gesturestart",gestureStart,!1)}function gestureStart(){for(i=0;i<metas.length;i++)"viewport"==metas[i].name&&(metas[i].content="width=device-width, minimum-scale=0.25, maximum-scale=1.6")}
...
a=0;a<d.length;a+=1)-1!==d[a].indexOf("/")?0!==a&&(a!==d.length-1&&!/\s+\/|\/\s+/.test(d[a]))&&(d[a]+="\u200b"):d[a].length>c&&(d[a]=this.hyphenate(d[a]).join("\u00ad"));return d.join("")};e.prototype.hyphenate=function(b){var c,d=[],a,e,h,g=[],f,l,k,n=Math.max,p=this.trie,m=[""];if(this.exceptions.hasOwnProperty(b.toLowerCase()))return b.match(this.exceptions[b.toLowerCase()]);if(-1!==b.indexOf("\u00ad"))return[b];b="_"+b+"_";c=b.toLowerCase().split("");b=b.split("");f=c.length;for(a=0;a<f;a+=1)g[a]=
0,d[a]=c[a].charCodeAt(0);for(a=0;a<f;a+=1)for(h=p,c=a;c<f;c+=1)if(h=h[d[c]]){if(l=h._points)for(e=0,k=l.length;e<k;e+=1)g[a+e]=n(g[a+e],l[e])}else break;for(a=1;a<f-1;a+=1)a>this.leftMin&&a<f-this.rightMin&&g[a]%2?m.push(b[a]):m[m.length-1]+=b[a];return m};window.Hypher=e;window.Hypher.languages={}})();
