 ...
 *
 * NOTE: this inserts soft hyphens into text, it does not perform the
 * full typesetting as performed in the referenced typeset example.
 */
jQuery(function($) {
    ...
    $(this).find('p').each(function(index, element) {
      ...
      for (var i = 0, children = element.childNodes; i < children.length; i++) {
        if (children[i].nodeType === 3)
          words = words.concat(children[i].nodeValue.trim().split(/\s+/));
        else
          words.push(children[i].outerHTML);
      }
      ...
      words.forEach(function(html, index, array) {
        ...
        if (word.length > 6 && word.length == html.length)
          hyphenated = h.hyphenate(word, 'en');
        else
          hyphenated.push(html);

        hyphenatedWords.push(hyphenated.join('&shy;'));
      });
