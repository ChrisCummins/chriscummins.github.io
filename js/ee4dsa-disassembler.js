...
var word2instruction = function (word, address, idtSize, comment) {
  ...
  switch (intByte0) {
    ...
    case 3:
      ...
      break;
    case 4:
      var operations = ['seto', '0x' + bytes[1], '0x' + bytes[2],
        '0x' + bytes[3]];
      ops.comment = 'OUT[' + intByte1 + '] = (OUT[' + intByte1 + '] & 0x' +
          bytes[2] + ') ^ 0x' + bytes[3];
      break;
    case 5:
      var operations = ['tsti', '0x' + bytes[1], '0x' + bytes[2],
        '0x' + bytes[3]];
      ops.comment = 'SR[T] = (OUT[' + intByte1 + '] & 0x' + bytes[2] + ') ^ 0x'
          + bytes[3];
      break;
    case 6:
      var operations = ['call'];
    ...
    for (i = 0; i < lastInstruction; i++) {
      ...
      if (string.length) {
        if (i < idtSize) { // Interrupt descriptor
          idt.push(
              word2instruction(string, address++, idtSize, 'Interrupt vector '
                  + i));
        } else {           // Instruction
