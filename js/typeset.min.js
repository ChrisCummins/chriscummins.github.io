...
jQuery(function(b){var c=new Hypher(Hypher.en);b("section").each(function(){b(this).find("p").each(function(b,a){for(var d=[],h=[],g=0,f=a.childNodes;g<f.length;g++)3===f[g].nodeType?d=d.concat(f[g].nodeValue.trim().split(/\s+/)):d.push(f[g].outerHTML);d.forEach(function(a,b,d){b=[];d=document.createElement("div");d.innerHTML=a;d=d.textContent||d.innerText||"";6<d.length&&d.length==a.length?b=c.hyphenate(d,"en"):b.push(a);h.push(b.join("&shy;"))});a.innerHTML=h.join(" ")})})});
