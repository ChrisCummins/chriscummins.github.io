...
(function() {
  ...
  /*
   * Webcam image upload complete. Note that we must declare this in the global
   * namespace so that he can use it as a global hook.
   */
  window.webcamUploadComplete = function(url) {
    if (url.match(/^http:\/\//)) {
      setImage(url);
      webcam.reset();
      hideWebcamUpload();
      ...
      /* FIXME: better alert, please */
      alert('Oops! Something went wrong: ' + url);
    }
  };
   ...
   */
  $('#webcam-snap-button').click(function() {
    webcam.snap();
  });
