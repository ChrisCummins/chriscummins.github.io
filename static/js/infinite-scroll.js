...
init(){this.setupIntersectionObserver();this.preloadNextPage();}
...
this.renderPhotos(data);this.currentPage=nextPage;this.hasMore=data.has_next;this.retryCount=0;if(this.hasMore){this.preloadNextPage();}}catch(error){console.error('InfiniteScroll: Error loading page',error);this.handleError(error);}finally{this.isLoading=false;this.hideLoader();}}
...
handleError(error){this.hideLoader();if(this.retryCount<this.maxRetries){this.retryCount++;setTimeout(()=>{this.isLoading=false;this.loadNextPage();},1000*this.retryCount);}else{this.showErrorMessage(error);if(this.paginationElement){this.paginationElement.style.display='block';}}}
