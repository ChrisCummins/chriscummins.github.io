...
createPhotoElement(photo){const photoDiv=document.createElement('div');photoDiv.setAttribute('data-collection',photo.collection);photoDiv.id=photo.slug;photoDiv.className='photo-item loading';const gridDiv=document.createElement('div');gridDiv.className='photographs-grid';const imageColumn=document.createElement('div');const imageButton=document.createElement('button');imageButton.type='button';imageButton.className='photograph-overlay-trigger';imageButton.dataset.title=photo.title||'';imageButton.dataset.medium='digital scan from film negative';const slug=photo.slug||'';if(slug){imageButton.dataset.slug=slug;}
const overlayYear=this.extractPhotoYear(photo);if(overlayYear){imageButton.dataset.year=overlayYear;}
const fullscreenWebp=slug?`${this.siteUrl}/static/photographs/fullscreen/${slug}.webp`:'';const fullscreenJpg=slug?`${this.siteUrl}/static/photographs/fullscreen/${slug}.jpg`:'';if(fullscreenWebp){imageButton.dataset.fullWebp=fullscreenWebp;}
if(fullscreenJpg){imageButton.dataset.fullJpg=fullscreenJpg;}
if(photo.images&&photo.images.display_width&&photo.images.display_height){imageButton.dataset.width=photo.images.display_width;imageButton.dataset.height=photo.images.display_height;}
const picture=document.createElement('picture');const webpSource=document.createElement('source');webpSource.srcset=photo.images.display;webpSource.type='image/webp';picture.appendChild(webpSource);const img=document.createElement('img');img.src=photo.images.display_fallback;img.alt=photo.title;img.className='clickable-image';img.loading='lazy';if(photo.images&&photo.images.display_width&&photo.images.display_height){img.setAttribute('width',photo.images.display_width);img.setAttribute('height',photo.images.display_height);}
picture.appendChild(img);imageButton.appendChild(picture);imageColumn.appendChild(imageButton);const detailColumn=document.createElement('div');detailColumn.className='detail-info';const title=document.createElement('h3');const titleLink=document.createElement('a');titleLink.href=photo.url;const titleSpan=document.createElement('span');titleSpan.className='image-title';titleSpan.textContent=photo.title;titleLink.appendChild(titleSpan);title.appendChild(titleLink);detailColumn.appendChild(title);const metadataSection=this.buildMetadataSection(photo);if(metadataSection){detailColumn.appendChild(metadataSection);}
const buttonsDiv=document.createElement('div');buttonsDiv.className='button-row';const moreInfoBtn=document.createElement('a');moreInfoBtn.href=photo.url;moreInfoBtn.className='btn';moreInfoBtn.textContent='More Info';buttonsDiv.appendChild(moreInfoBtn);if(photo.shop_enabled&&photo.shop_url){const shopBtn=document.createElement('a');shopBtn.href=photo.shop_url;shopBtn.className='btn';shopBtn.textContent='Collect Print';buttonsDiv.appendChild(shopBtn);}
...
extractPhotoYear(photo){if(photo.metadata&&photo.metadata.date_photographed){const year=new Date(photo.metadata.date_photographed).getFullYear();return Number.isNaN(year)?'':year.toString();}
if(photo.date){const year=new Date(photo.date).getFullYear();return Number.isNaN(year)?'':year.toString();}
return'';}
