...
if(this.image){this.image.classList.add('is-initializing');}
...
this.visible=true;this.updateZoomBounds({resetSlider:true,forceMeasurement:true});window.requestAnimationFrame(()=>{this.updateZoomBounds({forceMeasurement:true});window.requestAnimationFrame(()=>{if(this.image){this.image.classList.remove('is-initializing');}});});if(typeof this.options.onOpen==='function'&&!options.silent){this.options.onOpen();}}
...
this.state.pointerActive=true;this.state.pointerId=event.pointerId;this.state.startX=event.clientX;this.state.startY=event.clientY;this.state.startOffsetX=this.state.x;this.state.startOffsetY=this.state.y;this.state.dragging=false;this.state.justDragged=false;if(this.image){this.image.classList.add('is-panning');}}
panImage(event){if(!this.state.pointerActive||event.pointerId!==this.state.pointerId){return;}
event.preventDefault();const rawDeltaX=event.clientX-this.state.startX;const rawDeltaY=event.clientY-this.state.startY;if(Math.abs(rawDeltaX)>2||Math.abs(rawDeltaY)>2){this.state.dragging=true;}
const renderScale=this.getRenderScale();const deltaX=rawDeltaX/renderScale;const deltaY=rawDeltaY/renderScale;this.state.x=this.state.startOffsetX+deltaX;this.state.y=this.state.startOffsetY+deltaY;this.limitPan();this.applyImageTransform();}
endPan(event){if(!this.state.pointerActive||(event&&event.pointerId!==this.state.pointerId)){return;}
this.state.pointerActive=false;this.state.justDragged=this.state.dragging;this.state.dragging=false;if(this.image){this.image.classList.remove('is-panning');}
